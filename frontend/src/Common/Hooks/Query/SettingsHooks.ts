...
export function useGetSettingsByGroup(groupName: string, options?: any) {
  const queryResult = useQuery(
    ["settings-get", groupName],
    async () => await API.settings.getGroup(groupName),
    options
  );
  return queryResult;
}
...
export function useUpdateSettings(options?: any) {
  const mutateResults = useMutation(
    async (settingsData: object) => await API.settings.update(settingsData),
    options
  );
...
export function useUpdateSettingsByGroup(options?: any) {
  const mutateResults = useMutation(
    async (data: { groupIndex: number; settingsData: object }) =>
      await API.settings.updateGroup(data.groupIndex, data.settingsData),
    options
  );
  return mutateResults;
}
